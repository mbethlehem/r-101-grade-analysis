...
```{r}
ggplot(grades_new, aes(y=Exam,x= Group)) + geom_boxplot() + xlab("Group") + ggtitle("distribution of Exam conditioned by group") + coord_flip()
```
...
```{r}
ggplot(grades_NEW, aes(x=Exam)) + geom_density() + facet_wrap(~absent)
```
### Alternative 2:

```{r}
inner_join(grades, absence_online, by=join_by("Id")) |> filter(!is.na(Exam)) |> ggplot(aes(x=Exam)) + geom_density() + facet_wrap(~absence)
```
