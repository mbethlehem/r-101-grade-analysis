...
### Alternative 1
```{r}
ggplot(grades_new, aes(x=Group, fill=as.factor(Exam))) + geom_bar(position="fill") + ylab("proportion") + ggtitle("distribution of Exam conditioned by group")
```
```{r}
ggplot(grades_new, aes(x=Exam)) + geom_density() + facet_wrap(~Group)
```
```
...
```{r}
ggplot(table, aes(x=P_missed, y=mean)) + geom_point() + geom_line()
```
